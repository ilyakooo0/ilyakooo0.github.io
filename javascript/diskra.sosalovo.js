...
function update() {
	...
	let kr3 = round(parseFloat($("#KR3").val()))
	let dz3 = round(parseFloat($("#DZ3").val()))
	let kr4 = round(parseFloat($("#KR4").val()))
	let dz4 = round(parseFloat($("#DZ4").val()))
	let mit = round(parseFloat($("#MIT").val()))
	
	if (isNaN(kr3) || isNaN(dz3) || isNaN(kr4) || isNaN(dz4) || isNaN(mit)) {
		return
	}
	
	let d = round((0.62*Math.min(dz3, dz4) + 0.38*Math.max(dz3, dz4)))
	
	let k = round(0.62*Math.min(kr3, kr4) + 0.38*Math.max(kr3, kr4))
	let o = Math.round(0.62*Math.min(d, k) + 0.38*Math.max(d, k))
